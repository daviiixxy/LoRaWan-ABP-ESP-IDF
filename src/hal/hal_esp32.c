...
void hal_esp32_configure_pins(spi_host_device_t host, uint8_t nss, uint8_t rxtx, uint8_t rst, uint8_t dio0,
                              uint8_t dio1)
{
    spi_host = host;
...
void init_io(void)
{
    ...
    gpio_config_t output_pin_config = {.pin_bit_mask = BIT64(pin_nss),
...
void hal_pin_rst(u1_t val)
{
    ...
    {
#if defined(CONFIG_LORAWAN_RESET_STATES_ASSERTED)
...
void init_spi(void)
{
    ...
    spi_device_interface_config_t spi_config = {
        .mode = 0,
        .clock_speed_hz = CONFIG_LORAWAN_SPI_FREQ,
        .command_bits = 0,
        .address_bits = 8,
        .spics_io_num = -1,
        .queue_size = 1,
...
void init_timer(void)
{
    esp_timer_create_args_t timer_config = {
        .callback = &timer_callback, .arg = NULL, .dispatch_method = ESP_TIMER_TASK, .name = "lmic_job"};
...
void hal_esp32_start_lmic_task(void)
{
    ...
    xTaskCreate(lmic_background_task, "lorawan_lmic", 1024 * 4, NULL, CONFIG_LORAWAN_BG_TASK_PRIO, &lmic_task);
